...
class SyndicatController extends AbstractController
{
    ...
    #[Route('/syndicat/{id}/details', name: 'frontend_reclamation_details', methods: ['GET'])]
    public function details(Request $request, Reclamation $reclamation, \App\Repository\User\UserRepository $userRepository): Response
    {
        $session = $request->getSession();
        $userSession = $session->get('user');
